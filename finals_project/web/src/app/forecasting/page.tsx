  ...
  return (
   <AdminLayout title="Forecasting" subtitle="Inventory forecasts and reorder planning" headerActions={<Button variant="secondary"><Download className="w-4 h-4" />Export</Button>} onSearch={setSearch}>
    <div className="grid grid-cols-4 gap-4 mb-6">
     <KpiCard label="Need Reorder" value={atRisk.length} icon="AlertTriangle" change="below reorder point" changeType="negative" trend="up" onClick={()=>setKpiModal("reorder")} />
     <KpiCard label="Insufficient Stock" value={critical.length} icon="AlertTriangle" change="urgent" changeType="negative" trend="up" onClick={()=>setKpiModal("insufficient")} />
     <KpiCard label="Forecast Model" value="Holt-Winters" icon="TrendingUp" change="time series model" changeType="neutral" trend="stable" onClick={()=>setKpiModal("model")} />
     <KpiCard label="Forecast" value="7 days" icon="TrendingUp" change="next 7 days demand" changeType="neutral" trend="stable" onClick={()=>setKpiModal("forecast")} />
    </div>
