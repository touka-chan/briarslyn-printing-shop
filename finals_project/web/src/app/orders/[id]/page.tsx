...
const STEP_DATES: Record<Step, (o: Order) => string> = {
 Pending: (o) => o.createdAt ?? o.target_date,
 "In Production": (o) => o.createdAt ?? o.target_date,
...
export default function OrderDetailPage() {
 ...
 const initial = mockOrders.find((o) => o.order_id === id);
 const [status, setStatus] = useState<Step | undefined>(initial?.status as Step);

 useEffect(() => {
  if (initial) setStatus(initial.status as Step);
 }, [initial]);
    ...
    <ContentCard title="Activity">
     <div className="mt-2">
      ...
      <p className="text-sm flex items-center gap-2">
       ...
       {order.createdAt ?? order.target_date}
      </p>
     </div>
     <div className="mt-5 pt-5 border-t border-printflow-outline-variant/40 space-y-2">
      ...
      <Button
       variant="primary"
       onClick={advance}
       disabled={!status || currentIdx >= STEPS.length - 1}
       className="w-full"
      >
