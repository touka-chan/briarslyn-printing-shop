...
export default function InventoryPage() {
 ...
 const filtered = active==="All" ? mockInventory : mockInventory.filter(i=>i.status===active);
 const searched = filtered.filter(i=> !search || `${i.material_variant_id} ${i.item_type}`.toLowerCase().includes(search.toLowerCase()));
 const alerts = mockInventory.filter(i=> i.current_stock <= i.reorder_point);
 const stale = mockInventory.filter(i=> i.isStale);
 const lowStock = mockInventory.filter(i=> i.status==="Low Stock");
 const insufficient = mockInventory.filter(i=> i.status==="Insufficient Stock");
 ...
 const cols = [
  ...
  return (
   ...
   <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
    <KpiCard label="Total Variants" value={mockInventory.length} icon="Package" onClick={()=>setKpiModal("total")} />
    <KpiCard label="Low Stock" value={lowStock.length} icon="AlertTriangle" change="needs attention" changeType="negative" trend="up" onClick={()=>setKpiModal("low")} />
    <KpiCard label="Insufficient" value={insufficient.length} icon="AlertTriangle" change="urgent" changeType="negative" trend="up" onClick={()=>setKpiModal("insufficient")} />
    <KpiCard label="Need Reorder" value={alerts.length} icon="AlertTriangle" change={`${stale.length} delayed`} changeType={stale.length?"negative":"positive"} trend="up" onClick={()=>setKpiModal("reorder")} />
   </div>

    <Modal isOpen={kpiModal==="total"} onClose={()=>setKpiModal(null)} title="Total Variants" description={`${mockInventory.length} variants • GET /api/inventory`} icon={<Package className="w-5 h-5" />} size="lg" footer={<Button variant="secondary" onClick={()=>setKpiModal(null)}>Close</Button>}>
     <DataTable columns={cols} data={mockInventory} keyExtractor={r=>r.material_variant_id} emptyMessage="No materials" />
    </Modal>
